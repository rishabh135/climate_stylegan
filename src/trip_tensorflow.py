...
class TRIP(nn.Module):
    """
    Class for a Tensor Ring Learnable Prior(TRIP).
    """
    def __init__(self, distr_descr, tt_int=30, distr_init='uniform', eps=1e-10):
        """
        Args:
        ...
        for var_descr in self.distr_descr:
            if distr_init == 'rand':
                cur_core = tf.random_normal(var_descr[1], self.tt_int, self.tt_int)
            ...
            cur_core_paramter = tf.variable( initial_value=cur_core,name="cur_core_parameter", trainable=True)

            self.tt_cores.append(cur_core_paramter)
            ...
            if var_descr[0] == 'd':  # discrete variable
                ...
                if distr_init == 'rand':
                    cur_means = tf.random_uniform(var_descr[1]) * (rb - lb) + lb
                elif distr_init == 'uniform':
                    cur_means = (tf.range(var_descr[1]).float() /
                                 (var_descr[1] - 1)) * (rb - lb) + lb

                cur_log_stds = 2 * tf.log(
                    tf.ones(var_descr[1]) * (rb - lb) / var_descr[1]
                )

                tmp_mean = tf.variable( initial_value=cur_mean, name="cur_mean", trainable=True)
                tmp_stds = tf.variable( initial_value=cur_log_stds, name="cur_log_stds", trainable=True)
                
                self.means.append(tmp_mean)
                self.log_stds.append(tmp_stds)
    ...
    def __make_contr_vec(self, x, var, missed, means, log_stds):
        if missed is None:
        ...
        if var[0] == 'd':
            ...
            contr_vect = contr_vect + self.eps
    ...
    def log_prob(self, x, marg=None):
        ...
        pref = torch.eye(self.tt_cores[0].shape[1])
        pref = pref[None, :, :].repeat(num_objects, 1, 1)
        norm_pref = torch.eye(self.tt_cores[0].shape[1])

        pref = pref.to(x.device)
        norm_pref = norm_pref.to(x.device)

        for i, (core, var) in enumerate(zip(self.tt_cores, self.distr_descr)):
            ...
            if marg[i]:
                ...
                cur_contr_vect = self.__make_contr_vec(x[:, i],
                                                       var, None,
                                                       self.means[i],
                                                       self.log_stds[i])
                cond_core = core[None, :, :, :]
        ...
        eye = torch.eye(self.tt_cores[-1].shape[-1])[None, :, :]
        eye = eye.to(x.device)
    ...
    def sample(self, num_samples, sample_descr=None, conds=None):
        ...
        if sample_descr is None:
            sample_descr = len(self.distr_descr) * ['s']

        # computing contraction vectors
        contr_vect_list = []
        for i, (action, var) in enumerate(
                zip(sample_descr, self.distr_descr)):
            if action == 'c':
                contr_vect_list.append(self.__make_contr_vec(conds[:, i],
                                                             var,
                                                             None,
                                                             self.means[i],
                                                             self.log_stds[i]))
        ...
        # sampling
        pref = torch.eye(self.tt_cores[0].shape[1])[None, :, :]
        ...
        for i, (action, var, core, suf, prev_contr_vect) in enumerate(
            ...
            if action == 's':
                ...
                if var[0] == 'd':
                    ...
                    cur_means = self.means[i][discr_comp_sample]
                    cur_log_stds = self.log_stds[i][discr_comp_sample]
        ...
        return torch.cat(
            [s.float()[:, None] for s in samples_list if
             s is not None], dim=-1).detach()

    def reinit_from_data(self, data):
        """
        ...
        for i in range(len(self.tt_cores)):
            self.tt_cores[i].data = new_tt_cores[i].data.to(self.tt_cores[i].data.device)
            if new_means[i] is not None:
                self.means[i].data = new_means[i].data.to(self.means[i].data.device)
                self.log_stds[i].data = new_log_stds[i].data.to(self.log_stds[i].data.device)


    def stabilize(self):
        for r in range(len(self.tt_cores)):
            self.tt_cores[r].data = torch.abs(self.tt_cores[r].data) + self.eps
            norm = torch.sqrt(torch.sum(self.tt_cores[r].data ** 2))
            self.tt_cores[r].data = self.tt_cores[r].data / norm
