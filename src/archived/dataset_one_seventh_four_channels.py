...
ux_data_whole_max = ds['u_x'][st_time : end_time].values[:, :, :image_size].max()
uy_data_whole_max = ds['u_y'][st_time : end_time].values[:, :, :image_size].max()
max_value = max(ux_data_whole_max, uy_data_whole_max)

max_ds = ds["press_diff"]
...
for start_time in range(st_time, end_time, number_of_timesteps):
	...
	u_x = ux_data[:,:, :image_size]
	u_y = uy_data[:,:, :image_size]
